...
RUNNER_VARIANT = "Football Quiz MCQ"
...
# Per-question / per-answer multiple-choice voices (read aloud at play time).
MCQ_VOICE_DIR = PROJECT_ROOT / ".Storage" / "Voices" / "MCQ" / RUNNER_VARIANT
MCQ_VOICE_KINDS = ("questions", "answers")
QUIZ_TITLE_VOICE_FILE_BY_QUIZ_TYPE = {
    "english": {
        ...
        "football-quiz-mcq": "The ultimate football quiz - pick the right answer !!!.mp3",
    },
    "spanish": {
        "player-by-career": "Adivina al jugador por trayectoria !!!.mp3",
        "player-by-fake-info": "Adivina la informacion falsa del jugador !!!.mp3",
        "football-quiz-mcq": "El mejor quiz de futbol - elige la respuesta correcta !!!.mp3",
    },
}
QUIZ_TITLE_PROMPT_BY_QUIZ_TYPE = {
    "english": {
        "player-by-career": "Hey everyone, let's start. ... GUESS THE FOOTBALL PLAYER BY CAREER PATH!!",
        "player-by-fake-info": "Hey everyone, let's start. ... GUESS THE FAKE INFORMATION ABOUT THE PLAYER!!",
        "football-quiz-mcq": "Hey everyone, let's start. ... THE ULTIMATE FOOTBALL QUIZ! PICK THE RIGHT ANSWER!!",
    },
    "spanish": {
        "player-by-career": "Hola a todos, empecemos. ... ¡¡ADIVINA AL JUGADOR POR TRAYECTORIA!!",
        "player-by-fake-info": "Hola a todos, empecemos. ... ¡¡ADIVINA LA INFORMACIÓN FALSA DEL JUGADOR!!",
        "football-quiz-mcq": "Hola a todos, empecemos. ... ¡¡EL MEJOR QUIZ DE FÚTBOL! ELIGE LA RESPUESTA CORRECTA!!",
    },
}
...
class RunnerRequestHandler(SimpleHTTPRequestHandler):
    ...
    def _normalize_mcq_voice_inputs(self, kind, text, language):
        import unicodedata
        kind = str(kind or "").strip().lower()
        if kind not in MCQ_VOICE_KINDS:
            raise ValueError("kind must be one of %s" % (MCQ_VOICE_KINDS,))
        text = str(text or "").strip()
        if not text:
            raise ValueError("text is required")
        lang = _normalize_language(language)
        # Slug must match js/mcq-mode.js mcqSlug() exactly.
        s = unicodedata.normalize("NFD", text).encode("ascii", "ignore").decode("ascii").lower()
        s = re.sub(r"[^a-z0-9]+", "-", s).strip("-")[:80] or "untitled"
        out_path = MCQ_VOICE_DIR / lang / kind / (s + ".mp3")
        return text, out_path

    def _try_serve_mcq_voice_status(self) -> bool:
        parsed = urlparse(self.path)
        if parsed.path.rstrip("/") != "/__mcq-voice/status":
            return False
        query = {}
        for part in parsed.query.split("&"):
            if not part:
            ...
            query[unquote(k)] = unquote(v.replace("+", " "))
        try:
            _text, out_path = self._normalize_mcq_voice_inputs(
                query.get("kind"), query.get("text"), query.get("language"),
            )
        except ValueError as exc:
            self._send_json(400, {"ok": False, "error": str(exc)})
            return True
    ...
    def _try_generate_mcq_voice(self) -> bool:
        parsed = urlparse(self.path)
        if parsed.path.rstrip("/") != "/__mcq-voice/generate":
            return False
        try:
            body = self._read_json_body()
            prompt_text, out_path = self._normalize_mcq_voice_inputs(
                body.get("kind"), body.get("text"), body.get("language"),
            )
            requested_voice = str(body.get("voice") or FIXED_QUIZ_TITLE_VOICE).strip()
        except ValueError as exc:
            self._send_json(400, {"ok": False, "error": str(exc)})
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if self._try_serve_quiz_title_voice_status():
            return
        if self._try_serve_mcq_voice_status():
            return
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        if self._try_delete_quiz_title_voice():
            return
        if self._try_generate_mcq_voice():
            return

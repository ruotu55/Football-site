 ...
 */
export function playVoiceFromCandidates(candidates, delayMs = 1000) {
  const list = (candidates || []).filter((s) => !!s);
  if (list.length === 0) return Promise.resolve();
...
/** Sticky per-level phrase pick. Stored on the level object itself so the voice tab and
    ...
    The cache also re-rolls when the cached phrase is the "wrong kind" sentence
    (e.g. cached `team-is` but `kind` is now player). */
...
/** Build candidate URLs for one reveal, given the chosen phrase + kind. Tries chosen
    phrase first (current language → English), then falls through to plain so playback
    never goes silent when the user hasn't generated the sentence clip yet. For English
    plain only, also tries the legacy flat-root layout so pre-existing clips still resolve. */
...
export function playPlayerNameVoiceIfExists(displayName, delayMs = 0, kind) {
  ...
  if (candidates.length === 0) {
    playPlayerNameVoiceIfExistsInDir(displayName, delayMs, dir);
    return;
  }

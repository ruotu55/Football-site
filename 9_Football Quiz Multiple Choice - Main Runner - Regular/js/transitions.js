...
/* wc-sweep — World Cup Sweep (diagonal gold light-bar wipe).
   A skewed gold panel sweeps in from the left to fully cover the stage (content
   swaps under cover), then continues off to the right to reveal the next level.
   A brighter "shine" bar races ahead of the panel for a trophy-glint feel. */
registry.set("wc-sweep", {
  name: "World Cup Sweep",
  run: _newRun("wcsweep-overlay",
    (c) => { c.appendChild(_mkDiv("wcsweep-panel")); c.appendChild(_mkDiv("wcsweep-shine")); },
    (gsap, c) => {
      const panel = c.querySelector(".wcsweep-panel");
      const shine = c.querySelector(".wcsweep-shine");
      gsap.set(panel, { xPercent: -135 });
      gsap.set(shine, { xPercent: -180, opacity: 0.95 });
      return _tl(gsap, t => {
        t.to(panel, { duration: NEW_DUR * 0.55, xPercent: 0, ease: "power3.inOut" }, 0);
        t.to(shine, { duration: NEW_DUR * 0.72, xPercent: 150, ease: "power2.out" }, 0);
      });
    },
    (gsap, c) => {
      const panel = c.querySelector(".wcsweep-panel");
      const shine = c.querySelector(".wcsweep-shine");
      return _tl(gsap, t => {
        t.to(panel, { duration: NEW_DUR * 0.55, xPercent: 135, ease: "power3.inOut" }, 0);
        t.to(shine, { duration: NEW_DUR * 0.72, xPercent: 340, ease: "power2.in" }, 0);
      });
    }
  ),
});

...
const VOICE_FREEZE_MIGRATION_FLAG = "footballQuizVoiceFreezeMigrated_fake_information_regular_v2";
...
const KEY_SCRIPTS = "footballQuizScripts_football_quiz_mcq_regular_v1";
const KEY_FOLDERS = "footballQuizFolders_football_quiz_mcq_regular_v1";
const KEY_FOLDER_STATES = "footballQuizFolderStates_football_quiz_mcq_regular_v1";
...
const SAVE_SERVER = createSavedScriptsServerSync("football_quiz_mcq_regular", {
    KEY_SCRIPTS,
    KEY_FOLDERS,
    KEY_FOLDER_STATES,
});
...
export function setActiveScriptName(name) { activeScriptName = name == null ? null : String(name); }
...
export function captureCurrentScriptObject(name) {
    ...
    const levelsToSave = appState.levelsData.map((lvl) => ({
        ...
        mcq: jsonSafeClone(lvl.mcq) || null,
...
export async function applyScriptObject(script) { return loadScript(script); }

/** Build a script object from a "[Player1, Player2, ...]" paste � the same flow
 ...
 *  names are resolved by taking the first candidate (no disambiguation UI). */
...
function findAllPlayerCandidates(name, allPlayers) {
    ...
    /* Final fallback: require every query token to appear as a full token in the
       candidate name. This avoids broad substring matches that can resolve to
       unrelated entries. */
    hits = allPlayers.filter((p) => {
        if (!p?.name) return false;
        const parts = normalizeForImport(p.name).split(" ").filter(Boolean);
        if (parts.length === 0) return false;
        return queryTokens.every((token) => parts.includes(token));
    });
    return hits;
}

...
import { isMcqQuiz, renderMcqQuestion } from "./mcq-mode.js";
import {
  appState,
...
/* Smooth fade swap between the "?" placeholder and the revealed player name.
   Inline CSS-transition driven (more predictable than Web Animations API for
   this case). Monotonic token guards against rapid back-to-back swaps. The
   reflow flush in phase 3 is required: without it the browser coalesces the
   transition rule update + opacity flip into a single style write and the
   fade-in won't animate. */
const MYSTERY_FADE_OUT_MS = 120;
const MYSTERY_FADE_IN_MS = 280;
function fadeSwapMysteryText(el, newText, isName) {
  if (!el) return;
  const newTextStr = String(newText);
  const currentlyName = el.classList.contains("career-portrait-card__mystery-mark--name");
  if ((el.textContent || "") === newTextStr && currentlyName === !!isName) {
    return;
  }
  const myToken = (el._mysteryFadeToken || 0) + 1;
  el._mysteryFadeToken = myToken;

  el.style.transition = `opacity ${MYSTERY_FADE_OUT_MS}ms ease-in`;
  el.style.opacity = "0";

  setTimeout(() => {
    if (el._mysteryFadeToken !== myToken || !el.isConnected) return;
    el.textContent = newTextStr;
    if (isName) {
      el.classList.add("career-portrait-card__mystery-mark--name");
      autoFitMysteryName(el);
    } else {
      el.classList.remove("career-portrait-card__mystery-mark--name");
      el.style.fontSize = "";
    }
    ...
    void el.offsetWidth;
    el.style.transition = `opacity ${MYSTERY_FADE_IN_MS}ms ease-out`;
...
/* Format the revealed player name for the mystery-mark: first name on the
   first line, the rest on the second line. Single-word names stay one line.
   Long hyphenated surnames (e.g. ALEXANDER-ARNOLD) use a third line at the
   hyphen so the bar does not auto-wrap mid-word. The "\n" is rendered as a
   real line break by the `white-space: pre-line` rule on
...
async function postReadyPhotoAction(endpoint, payload, timeoutMs) {
  ...
  try {
    ...
    if (e && e.name === "AbortError") {
      throw new Error(`Request took longer than ${timeoutMs / 1000}s and was cancelled.`);
    }
...
async function requestRemoveReadyPhotoBackground(playerName, clubName, variantIndex) {
  return postReadyPhotoAction(
    "/__ready-photo/remove-bg",
    { playerName, clubName: clubName ?? "", variantIndex: variantIndex ?? 1 },
    180000,
  );
}

async function requestDeleteReadyPhoto(playerName, clubName, variantIndex) {
  return postReadyPhotoAction(
    "/__ready-photo/delete",
    { playerName, clubName: clubName ?? "", variantIndex: variantIndex ?? 1 },
    15000,
  );
}
...
function createCareerGetPhotoControls(playerName, clubName) {
  ...
  btnRemoveBg.className = "career-get-photo-btn career-get-photo-btn--remove-bg";
  btnRemoveBg.textContent = "Remove background";
  btnRemoveBg.title = "Run the rembg model on the current Ready photo and replace it with a transparent PNG.";
  btnRemoveBg.addEventListener("click", async () => {
    if (!careerReadyPhotoFetchServerActive()) {
      showHint("Run via run_site.py so the local server can edit Ready photos.", true);
      return;
    }
    const st = getState();
    ...
    try {
      ...
      showHint(e?.message || "Background removal failed.", true);
    } finally {
      btnRemoveBg.disabled = false;
      btnRemoveBg.textContent = prev;
    }
  });
  btnRow.appendChild(btnRemoveBg);
 ...
 * (1000ֳ—400 viewBox stretched to the wrap). Converting via clientWidth/height keeps portrait photos
...
function applyCareerSilhouetteAdjustments(silhouetteEl, st, { noExtraDown = false } = {}) {
  ...
  const finalScaleX =
    CAREER_SHADOW_UNIFORM_SCALE * safeScaleX * CENTER_PLAYER_IMAGE_SCALE_MULTIPLIER;
  const finalScaleY =
    CAREER_SHADOW_UNIFORM_SCALE * safeScaleY * CENTER_PLAYER_IMAGE_SCALE_MULTIPLIER;
...
function applyCareerRevealAdjustments(wrapEl, st) {
  ...
  const revealScaleX =
    CAREER_REVEAL_BASE_SCALE * safeScaleX * CENTER_PLAYER_IMAGE_SCALE_MULTIPLIER;
  const revealScaleY =
    CAREER_REVEAL_BASE_SCALE * safeScaleY * CENTER_PLAYER_IMAGE_SCALE_MULTIPLIER;
  const applyVars = (el) => {
...
export function shouldFourParamsVmOffPostReveal(state = getState()) {
  if (!state || document.body.classList.contains("shorts-mode")) return false;
...
export function renderCareer() {
  const state = getState();
  const stateHasMcq = !!(state && state.mcq && Array.isArray(state.mcq.answers) && state.mcq.answers.length);
  if (isMcqQuiz() || stateHasMcq) {
    /* Multiple-choice quiz: render the A/B/C question UI and skip the entire
       career/silhouette pipeline below. The `stateHasMcq` fallback keeps a level
       with question data rendering even if the quiz-type <select> is momentarily
       out of sync (e.g. mid-load). */
    renderMcqQuestion(state);
    return;
  }
  const hasRunLegacyPictureMigration =
    !!(state && state.__legacyRegularPictureMigrationDone);
  ...
  if (!isShorts && !hasRunLegacyPictureMigration) {
  ...
  if (!hasRunLegacyPictureMigration) {
    /* Run legacy default migration only once per level state. Without this guard,
       a user setting width back to 0.85 in Adjust Picture gets mistaken for
       "old defaults" and forced back to 1.0 on every re-render. */
    state.__legacyRegularPictureMigrationDone = true;
  }
  ...
  document.getElementById("career-delete-image-btn")?.remove();
  ...
  if (hasRealPlayer) {
    ...
    const voiceChromeMountEarly = document.getElementById("player-voice-chrome-mount");
    if (voiceChromeMountEarly) {
      voiceChromeMountEarly.insertBefore(clearBtn, voiceChromeMountEarly.firstChild);
      if (getPhotoUi) {
        voiceChromeMountEarly.appendChild(getPhotoUi.host);
        if (careerGetPhotoSuppressed()) getPhotoUi.hide();
      }
    } else {
      wrap.appendChild(clearBtn);
      if (isShorts && getPhotoUi) {
    ...
    deleteImageBtn.id = "career-delete-image-btn";
    deleteImageBtn.className = "career-delete-image-btn";
    deleteImageBtn.textContent = "Delete image";
    deleteImageBtn.title = "Delete the currently displayed Ready photo file from disk.";
    deleteImageBtn.addEventListener("click", async (e) => {
      e.preventDefault();
      e.stopPropagation();
      if (!careerReadyPhotoFetchServerActive()) return;
      const st = getState();
      const variantIndex = Math.max(1, Math.floor(Number(st?.careerReadyPhotoVariantIndex) || 1));
      ...
      try {
        ...
        deleteImageBtn.textContent = prev;
        deleteImageBtn.disabled = false;
        console.error("[Delete image]", err);
      }
    });
    if (voiceChromeMountEarly) {
      voiceChromeMountEarly.insertBefore(deleteImageBtn, clearBtn);
    } else {
      wrap.appendChild(deleteImageBtn);
    }
  }
  ...
  if (isShorts) {
    ...
    if (hasRealPlayer) {
      ...
      /* Controls row above the portrait: (Clear team) + (Voice play) + (Voice delete), centered. */
      const playerControlsRow = document.createElement("div");
      playerControlsRow.className = "career-player-controls-row";

      const voiceM = document.getElementById("player-voice-chrome-mount");
      const clearPlayerBtn = document.getElementById("career-clear-player-btn");
      if (clearPlayerBtn && (!voiceM || !voiceM.contains(clearPlayerBtn))) {
        playerControlsRow.appendChild(clearPlayerBtn);
      }
      if (!isShorts && getPhotoUi && (!voiceM || !voiceM.contains(getPhotoUi.host))) {
        playerControlsRow.appendChild(getPhotoUi.host);
        if (careerGetPhotoSuppressed()) getPhotoUi.hide();
      }

      paramCluster.appendChild(playerControlsRow);
    ...
    panel.addEventListener("click", (e) => {
      ...
      if (portraitPhoto) {
        ...
        if (portraitCardEl?.classList.contains("career-portrait-card--vm-off-revealed")) {
          ...
          const finalSx =
            CAREER_SHADOW_UNIFORM_SCALE * safeSx * CENTER_PLAYER_IMAGE_SCALE_MULTIPLIER * 2;
          const finalSy =
            CAREER_SHADOW_UNIFORM_SCALE * safeSy * CENTER_PLAYER_IMAGE_SCALE_MULTIPLIER * 2;

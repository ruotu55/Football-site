...
const VOICE_FREEZE_MIGRATION_FLAG = "footballQuizVoiceFreezeMigrated_career_shorts_v2";
...
export async function buildScriptFromImportText(text, name) {
    ...
    // Headless flow: no interactive disambig modal — if ambiguous, fail with a
    // clear error so the user can edit the paste to be more specific.
    if (ambiguous.length > 0) {
        const ambigErrors = ambiguous.map((a) => {
            const labels = a.candidates
                .slice(0, 5)
                .map((p) => `${p.name} (${p?._clubItem?.name || "?"})`)
                .join(", ");
            return `❌ ${a.rawName}: ambiguous (${a.candidates.length} matches: ${labels}${a.candidates.length > 5 ? ", …" : ""}).`;
        });
        const ambigSearchable = new Set(ambiguous.map((a) => a.rawName));
        return { ok: false, errors: ambigErrors, searchableNames: ambigSearchable };
    }

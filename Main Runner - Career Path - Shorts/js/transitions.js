...
class GridOverlay {
  ...
  async show(gsap, cfg = {}) {
    ...
    // Kill any leftover tweens from a previous transition
    const cellEls = this.cells.map(c => c.el);
    gsap.killTweensOf(cellEls);
...
registry.set("curtain-close", {
  ...
  async run(updateContentFn) {
    ...
    gsap.killTweensOf([topHalf, bottomHalf]);
...
/** Force-hide all transition overlay containers so nothing stays stuck on screen. */
function forceHideAllOverlays() {
  const selectors = [
    ".grid-transition-overlay",
    ".bars-transition-overlay",
    ".curtain-transition-overlay",
    ".skew-transition-overlay",
  ];
...
export async function runTransition(updateContentFn) {
  ...
  try {
    ...
    console.error("Transition run error:", e);
    forceHideAllOverlays();
  } finally {

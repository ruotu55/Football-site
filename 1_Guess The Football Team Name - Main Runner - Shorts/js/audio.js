...
const RUNNER_VARIANT = "Lineups Shorts";
...
/** Build a fallback chain: preferred-language first, English as safety net.
    Each resolver takes its domain args first and `lang` LAST ג€” this helper
 ...
 * Does not call `__resolveQuizTitleVoiceSrc` ג€” `clipSrc` must be a usable URL string.
 */
...
/**
 * Bundled ג€Game nameג€ MP3 only (no async API). Use for Play Video from landing so the line always plays.
...
/** Build the candidate chain for one reveal, given the chosen phrase. Tries the chosen
    phrase first (current language ג†’ English), then falls through to plain so playback

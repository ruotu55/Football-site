...
#   & "C:\Users\Rom\Desktop\‏‏תיקיה חדשה\Football Channel\Main Runner - Lineups - Regular\run_site.bat" --no-browser
# macOS/Linux:
#   python3 "C:/Users/Rom/Desktop/‏‏תיקיה חדשה/Football Channel/Main Runner - Lineups - Regular/run_site.py"
...
COQUI_VOICE_OPTIONS = {
    "coqui-jenny": {"model_name": "tts_models/en/jenny/jenny"},
    "coqui-ljspeech-glow": {"model_name": "tts_models/en/ljspeech/glow-tts"},
    "coqui-ljspeech-tacotron2": {"model_name": "tts_models/en/ljspeech/tacotron2-DDC"},
    "coqui-vctk-p225": {"model_name": "tts_models/en/vctk/vits", "speaker": "p225"},
    "coqui-vctk-p230": {"model_name": "tts_models/en/vctk/vits", "speaker": "p230"},
}
DEFAULT_COQUI_VOICE = "coqui-jenny"
...
_COQUI_TTS_CACHE: dict[str, object] = {}
...
def _edge_tts_command() -> list[str] | None:
    # Try module form first so Windows virtualenvs work reliably.
...
def _resolve_coqui_voice_key(requested_voice: str) -> str:
    key = str(requested_voice or "").strip().lower()
    return key if key in COQUI_VOICE_OPTIONS else DEFAULT_COQUI_VOICE


def _coqui_voice_config(voice_key: str) -> dict:
    return COQUI_VOICE_OPTIONS[_resolve_coqui_voice_key(voice_key)]


def _get_cached_coqui_tts(model_name: str):  # noqa: ANN001
    with _COQUI_TTS_LOCK:
        if model_name in _COQUI_TTS_CACHE:
            return _COQUI_TTS_CACHE[model_name]
        from TTS.api import TTS  # type: ignore

        engine = TTS(model_name=model_name, progress_bar=False, gpu=False)
        _COQUI_TTS_CACHE[model_name] = engine
        return engine


def _generate_coqui_speech_wav(text: str, voice_key: str, out_path: Path) -> None:
    if not _coqui_tts_available():
        raise RuntimeError("Coqui TTS is not installed. Install it with: pip install TTS")
    cfg = _coqui_voice_config(voice_key)
    model_name = str(cfg.get("model_name") or "").strip()
    if not model_name:
        raise RuntimeError("Invalid Coqui voice config.")
    speaker = str(cfg.get("speaker") or "").strip()
    try:
        engine = _get_cached_coqui_tts(model_name)
        kwargs = {}
        if speaker:
            kwargs["speaker"] = speaker
        engine.tts_to_file(text=str(text or ""), file_path=str(out_path), **kwargs)
    except Exception as exc:  # noqa: BLE001
        raise RuntimeError(f"Coqui generation failed: {exc}") from exc
    if not out_path.exists() or out_path.stat().st_size <= 0:
        raise RuntimeError("Coqui returned empty audio.")
...
def _video_export_clock_tick_ms(delta_ms: float | int) -> int:
    """Playwright treats numeric clock ticks as milliseconds; strings are human time."""
    return max(0, int(round(float(delta_ms))))


# Smaller `run_for` steps give GSAP/CSS more intermediate ticks per encoded frame.
VIDEO_EXPORT_PLAYWRIGHT_CLOCK_STEP_MS = 8


def _safe_video_export_audio_media_path(project_root: Path, rel: str) -> Path | None:
    """Application Security Requirement: allowlist `.Storage/` and `Images/` only; reject traversal."""
    rel_norm = unquote(str(rel or "")).replace("\\", "/").strip().lstrip("/")
    if not rel_norm:
        return None
    for part in rel_norm.split("/"):
        if part in ("", ".", ".."):
            return None
    low = rel_norm.lower()
    if not (low.startswith(".storage/") or low.startswith("images/")):
        return None
    cand = (project_root / rel_norm).resolve()
...
def _mux_video_export_mp4_with_audio_cues(
    project_root: Path,
    video_in: Path,
    cues: list,
    duration_sec: float,
    ...
    """If ffmpeg succeeds, return path to MP4 with AAC audio; else None (caller keeps silent video)."""
    if not cues:
        return None
    ffmpeg_bin = shutil.which("ffmpeg")
    if not ffmpeg_bin:
        print("[video-export] ffmpeg not on PATH; skipping audio mux", flush=True)
        return None
    ...
    for raw in cues:
        if not isinstance(raw, dict):
            continue
        rel = str(raw.get("rel") or raw.get("path") or "").strip()
        p = _safe_video_export_audio_media_path(project_root, rel)
        if p is None:
            print(f"[video-export] skip missing or unsafe audio rel={rel!r}", flush=True)
            continue
        resolved.append((p, raw))
        if len(resolved) >= 48:
            print("[video-export] audio cue cap reached (48); truncating", flush=True)
            break
    ...
    fc_lines: list[str] = [
        f"anullsrc=channel_layout=stereo:sample_rate=44100:d={dur:.6f}[sil]",
    ]
    for i, (_p, cue) in enumerate(resolved):
        inp = i + 1
        try:
            vol = float(cue.get("volume", 1) or 1)
        except (TypeError, ValueError):
            vol = 1.0
        vol = max(0.0, min(1.5, vol))
        try:
            del_ms = max(0, int(round(float(cue.get("startMs", 0) or 0))))
        except (TypeError, ValueError):
            del_ms = 0
        trim = ""
        dm = cue.get("durationMs")
        if dm is not None:
            try:
                dsec = max(0.05, float(dm) / 1000.0)
                trim = f",atrim=duration={dsec:.6f}"
            except (TypeError, ValueError):
                trim = ""
        fc_lines.append(
            f"[{inp}:a]aformat=sample_rates=44100:channel_layouts=stereo,volume={vol:.6f}{trim},"
            f"adelay={del_ms}|{del_ms}[a{i}]"
        )

    mix_parts = "[sil]" + "".join(f"[a{i}]" for i in range(len(resolved)))
    fc_lines.append(f"{mix_parts}amix=inputs={len(resolved) + 1}:normalize=1:duration=first[outa]")

    script_path = work_dir / "video_export_audio_fc.txt"
    out_path = work_dir / "export_with_audio.mp4"
    script_path.write_text("\n".join(fc_lines), encoding="utf-8")

    cmd: list[str] = [ffmpeg_bin, "-y", "-hide_banner", "-loglevel", "error", "-i", str(video_in)]
    for p, _cue in resolved:
        cmd.extend(["-i", str(p)])
    cmd.extend(
        [
    ...
    try:
        ...
        print("[video-export] ffmpeg mux timed out; keeping silent video", flush=True)
        return None
    if proc.returncode != 0:
        err = (proc.stderr or proc.stdout or "").strip()
        print(f"[video-export] ffmpeg mux failed rc={proc.returncode}: {err[:800]}", flush=True)
        return None
    if not out_path.is_file() or out_path.stat().st_size <= 0:
        return None
    print(f"[video-export] muxed audio cues={len(resolved)} -> {out_path.stat().st_size} bytes", flush=True)
    return out_path


_RAF_WAIT_JS = "new Promise((r) => requestAnimationFrame(() => requestAnimationFrame(r)))"
_VIDEO_EXPORT_CLOCK_SYNC_JS = """(clockMs) => {
    if (typeof window.__pwVideoExportClockSync === "function") {
...
class RunnerRequestHandler(SimpleHTTPRequestHandler):
    ...
    def _try_rename_team_voice(self) -> bool:
        parsed = urlparse(self.path)
        if parsed.path.rstrip("/") != "/__team-voice/rename":
            return False
        try:
            body = self._read_json_body()
    ...
    def _try_player_photo_from_url(self) -> bool:
        ...
        try:
            ...
            self._write_json(400, {"ok": False, "error": f"Failed to download image: {exc}"})
            return True
        if not image_bytes:
            self._write_json(404, {"ok": False, "error": "Downloaded image is empty."})
            return True
        ...
        target_dir.mkdir(parents=True, exist_ok=True)
        out_path = _next_auto_photo_path(target_dir, "Manual")
        try:
            out_path.write_bytes(image_bytes)
        except OSError:
            self._write_json(500, {"ok": False, "error": "Failed to write image file."})
    ...
    def _try_render_video_export(self) -> bool:
        parsed = urlparse(self.path)
        if parsed.path.rstrip("/") != "/__video-export/render":
            return False
        try:
            body = self._read_json_body()
            ...
            if not snapshot:
                raise ValueError("Missing quiz snapshot.")
            json.loads(snapshot)
        ...
        try:
            video_bytes = self._render_video_export_mp4(snapshot, fps)
        except Exception as exc:  # noqa: BLE001
            traceback.print_exc()
            self._write_json(500, {"ok": False, "error": f"Offline video export failed: {exc}"})
            return True
    ...
    def _render_video_export_mp4(self, snapshot: str, fps: int) -> bytes:
        import imageio.v2 as imageio  # noqa: PLC0415
        import numpy as np  # noqa: PLC0415
        from PIL import Image  # noqa: PLC0415
        from playwright.sync_api import sync_playwright  # noqa: PLC0415

        snapshot_key = "lineups-regular-fcbnew-dev-live-reload-snapshot-v1"
        host = self.headers.get("Host") or f"127.0.0.1:{self.server.server_address[1]}"
        page_url = f"http://{host}{RUNNER_WEB_PREFIX}/index.html?video-export=1"
        frame_ms = 1000 / fps
        snapshot_data = json.loads(snapshot)
        total_levels = int(snapshot_data.get("totalLevelsCount") or 3)
        # Conservative deterministic cap: intro/landing + each playable question + outro.
        # This prevents a hidden browser/media state bug from rendering silently forever.
        estimated_questions = max(1, total_levels - 2)
        max_seconds = min(240, 14 + (estimated_questions * 9) + 8)
        max_frames = int(max_seconds * fps)
        print(
            f"[video-export] start offline render fps={fps} levels={total_levels} "
            f"snapshot_current_level={snapshot_data.get('currentLevelIndex')} "
            f"snapshot_levels_data={len(snapshot_data.get('levelsData') or [])} "
            f"frame_ms={frame_ms:.6f} max_seconds={max_seconds} max_frames={max_frames}",
            flush=True,
        )

        with tempfile.TemporaryDirectory(prefix="football-video-export-") as tmp_dir:
            out_path = Path(tmp_dir) / "export.mp4"
            audio_cues: list = []
            frames_written = 0
                ...
                with sync_playwright() as p:
                    ...
                    try:
                        ...
                        page.on(
                            "console",
                            lambda msg: print(
                                f"[video-export][browser:{msg.type}] {msg.text}",
                                flush=True,
                            ),
                        )
                        page.on(
                            "pageerror",
                            lambda exc: print(
                                f"[video-export][browser:pageerror] {exc}",
                                flush=True,
                            ),
                        )
                        page.clock.install()
                        page.add_init_script(
                            f"sessionStorage.setItem({json.dumps(snapshot_key)}, {json.dumps(snapshot)});"
                        )
                        print(f"[video-export] opening hidden page {page_url}", flush=True)
                        page.goto(page_url, wait_until="load", timeout=120_000)
                        page.wait_for_function(
                            "typeof window.__startOfflineVideoExportPlayback === 'function'",
                            timeout=120_000,
                        )
                        initial_status = page.evaluate(
                            """() => {
                                if (typeof window.__pwVideoExportClockSync === "function") {
                                    window.__pwVideoExportClockSync(0);
                                }
                                return window.__getOfflineVideoExportStatus();
                            }"""
                        )
                        print(
                            "[video-export] status before playback "
                            + json.dumps(initial_status, ensure_ascii=False, sort_keys=True),
                            flush=True,
                        )
                        page.evaluate("window.__startOfflineVideoExportPlayback()")
                        print("[video-export] hidden playback started", flush=True)

                        started = False
                        frames_written = 0
                        clock_elapsed_ms = 0
                        outro_guard_frames = 0
                        outro_cta_hold_frames = 0
                        for frame_index in range(max_frames):
                            target_elapsed_ms = round(frame_index * frame_ms)
                            advance_ms = max(0, target_elapsed_ms - clock_elapsed_ms)
                            if advance_ms:
                                while clock_elapsed_ms < target_elapsed_ms:
                                    step = min(
                                        VIDEO_EXPORT_PLAYWRIGHT_CLOCK_STEP_MS,
                                        target_elapsed_ms - clock_elapsed_ms,
                                    )
                                    if step <= 0:
                                        break
                                    page.clock.run_for(step)
                                    clock_elapsed_ms += step
                                    page.evaluate(_RAF_WAIT_JS)
                                    page.evaluate(
                                        _VIDEO_EXPORT_CLOCK_SYNC_JS,
                                        clock_elapsed_ms,
                                    )
                            ...
                            page.evaluate(
                                "() => {"
                                " if (window.__pwVideoExportSleepMotionForCapture) "
                            ...
                            page.evaluate(
                                "() => {"
                                " if (window.__pwVideoExportWakeMotionAfterCapture) "
                            ...
                            print(
                                "[video-export][frame] "
                                f"frame={frames_written}/{max_frames} "
                                f"target_ms={target_elapsed_ms} "
                                f"advance_ms={advance_ms} "
                                f"clock_elapsed_ms={clock_elapsed_ms} "
                                f"level={status.get('currentLevelIndex')}/{status.get('totalLevelsCount')} "
                                f"level_kind={status.get('levelKind')} "
                                f"playing={status.get('isVideoPlaying')} "
                                f"video_mode={status.get('videoMode')} "
                                f"has_squad={status.get('hasCurrentSquad')} "
                                f"active_audio={status.get('activeAudioCount')} "
                                f"audio_events={status.get('audioEventCount')} "
                                f"last_audio={status.get('lastAudioEvent')} "
                                f"outro_cta={status.get('outroCtaVisible')}",
                                flush=True,
                            )
                            if status.get("isVideoPlaying"):
                                started = True
                                if status.get("currentLevelIndex") == status.get("totalLevelsCount"):
                                    outro_guard_frames += 1
                                    if status.get("outroCtaVisible"):
                                        outro_cta_hold_frames += 1
                                        # End soon after like/subscribe finish popping in (CSS ~3s delays).
                                        hold_need = max(8, int(round(fps * 0.35)))
                                        if outro_cta_hold_frames >= hold_need:
                                            break
                                    else:
                                        outro_cta_hold_frames = 0
                                        if outro_guard_frames >= int(fps * 15):
                                            print(
                                                "[video-export] outro CTA visibility timeout; stopping",
                                                flush=True,
                                            )
                                            break
                                else:
                                    outro_guard_frames = 0
                                    outro_cta_hold_frames = 0
                            elif started and frames_written > fps:
                                break
                        else:
                            print(
                                f"[video-export] reached deterministic frame cap at {frames_written} frames",
                                flush=True,
                            )

                        if not started:
                            raise RuntimeError("Playback did not start in the offline renderer.")
                        if frames_written <= fps:
                            raise RuntimeError("Offline renderer stopped before enough frames were written.")
                        print(f"[video-export] finished frames={frames_written}", flush=True)
                        try:
                            audio_cues = page.evaluate(
                                "() => (Array.isArray(window.__footballVideoExportAudioCues) "
                                "? window.__footballVideoExportAudioCues : [])"
                            )
                        except Exception as exc:
                            print(f"[video-export] audio cue list read failed: {exc}", flush=True)
                            audio_cues = []
                    finally:
                        browser.close()
            finally:
            ...
            if not out_path.is_file() or out_path.stat().st_size <= 0:
                raise RuntimeError("MP4 encoder returned an empty file.")
            print(f"[video-export] encoded mp4 bytes={out_path.stat().st_size}", flush=True)
            duration_sec = max(1, int(frames_written)) / float(fps)
            muxed = _mux_video_export_mp4_with_audio_cues(
                PROJECT_ROOT,
                out_path,
                audio_cues,
                duration_sec,
            ...
            if muxed is not None and muxed.is_file() and muxed.stat().st_size > 0:
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        if self._try_player_photo_from_url():
            return
        if self._try_render_video_export():
            return

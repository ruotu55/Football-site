    ...
    try {
        return foldTurkishLatinForImport(
            str.trim()
                ...
                .replace(/ł/g, "l").replace(/Ł/g, "l")  // Polish: ł→l (Białystok→bialystok)
                .replace(/đ/g, "d").replace(/Đ/g, "d")  // Croatian/Serbian: đ→d
                .replace(/\//g, " ")         // slash → space (Bodø/Glimt → bodo glimt)
                .replace(/-/g, " ")          // hyphen → space (Al-Hilal == Al Hilal)
                .replace(/[''`´']/g, "")
                .replace(/\./g, "")
                .replace(/\s+/g, " ")
                .trim(),
 ...
 *  Covers the club list used by the channel so pasted names resolve EXACTLY
 *  instead of relying on the fragile substring fallback (which e.g. matched
 *  "Barcelona" → "Barcelona SC Guayaquil"). */

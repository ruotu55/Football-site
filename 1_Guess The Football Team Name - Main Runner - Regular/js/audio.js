...
/* Exposed for the Voice-tab crossfade PREVIEW tool so it uses the exact same song
   list + crossfade timing as the live player. */
export const BGM_PLAYLIST = paths.bgmPlaylist;
...
export const BGM_CROSSFADE_BUFFER_S = 0.15;
...
/** Build the candidate chain for one reveal, given the chosen phrase. Tries the chosen
    ...
    falls back to "plain" when a sentence phrase was assigned, because runtime would
    otherwise say only the team name instead of the selected sentence. */
export function buildRevealVoiceCandidates(displayName, quizType, phraseKey, language = getCurrentLanguage()) {
...
function buildRevealCandidates(displayName, quizType, phraseKey) {
  return buildRevealVoiceCandidates(displayName, quizType, phraseKey);
}
...
export function playTeamNameVoiceIfExists(displayName, quizType = "club-by-nat", delayMs = 0) {
  const stems = resolveTeamNameVoiceFileStems(displayName);
  if (!stems.length) return;
  const dir = revealVoiceDirForQuizType(quizType);
  const lang = getCurrentLanguage();

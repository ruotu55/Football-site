// js/thumbnail-studio.js — YouTube thumbnail generator (1280×720) for Runner 2 Regular.
//
...
//   • Top 25% red banner with "GUESS THE NATIONAL TEAM" (yellow accent on the
//     last two words), Impact font.
//   • "2025/6" season badge (vertical, top-right corner).
//   • Bottom 75% green pitch with a huge centered circular country flag and the
//     country name in giant Impact below it (e.g. "SPAIN").
//   • Optional secondary title bar (e.g. "World Cup") with auto-resolved
...
// country selection is sampled from. The composition stays static — no animation,
...
const EFFECTS = [
    ...
    "spotlight-center",      // bright spotlight behind the flag
];
...
function pickRandomQuestionLevelIdx() {
    ...
    for (let i = 0; i < levels.length; i++) {
        ...
        const c = pickCountryForCurrentLevel(lvl);
    ...
    overlay.innerHTML = `
        ...
        <aside class="ts-rail">
            ...
            <label class="ts-field">
                ...
                <input type="text" class="ts-input" data-field="specificTitle" placeholder="e.g. World Cup" />
            </label>
...
// ─── Country block — huge centered circular flag + country name ──────────
async function drawCountryBlock(ctx) {
    const levels = Array.isArray(appState.levelsData) ? appState.levelsData : [];
    const idx = state.sourceLevelIdx;
    const lvl = idx >= 0 && idx < levels.length ? levels[idx] : null;
    const country = pickCountryForCurrentLevel(lvl);
    const name = country?.name ? String(country.name).toUpperCase() : "";
    const flagSrc = country?.flagSrc || null;

    // Big circular flag — centered in the upper portion of the pitch area so the
    // country name fits below without crowding the optional specific-title pill.
    const cx = W / 2;
    const flagR = 180;                                   // 360px diameter
    const cy = PITCH_TOP + 40 + flagR;                   // top-aligned with breathing room

    // Outer white ring
    ctx.save();
    ctx.beginPath();
    ctx.arc(cx, cy, flagR + 10, 0, Math.PI * 2);
    ctx.fillStyle = "#FFFFFF";
    ctx.fill();
    ctx.restore();

    // Flag clipped into the circle
    ctx.save();
    ctx.beginPath();
    ...
    if (flagSrc) {
        try {
            ...
            const ratio = Math.max((flagR * 2) / img.width, (flagR * 2) / img.height);
            const w = img.width * ratio;
            const h = img.height * ratio;
            ctx.drawImage(img, cx - w / 2, cy - h / 2, w, h);
        } catch { /* fall through, ring stays white */ }
    } else {
    ...
    ctx.save();
    ctx.beginPath();
    ctx.arc(cx, cy, flagR + 6, 0, Math.PI * 2);
    ctx.lineWidth = 3;
    ctx.strokeStyle = "rgba(0,0,0,0.45)";
    ctx.stroke();
    ctx.restore();

    // Giant country name underneath, Impact, white with thick black stroke.
    if (name) {
        ctx.save();
        ...
        const maxW = W - 160;             // breathing room on sides
        const maxH = 110;                 // height budget below the flag
        let fontSize = maxH;
        do {
        ...
        const ty = cy + flagR + 16 + fontSize / 2;
        ctx.lineWidth = Math.max(6, fontSize * 0.10);
        ctx.strokeStyle = "#000000";
        ctx.strokeText(name, W / 2, ty);
        ctx.fillStyle = "#FFFFFF";
...
function pickCountryForCurrentLevel(lvl) {
    if (!lvl) return null;
    // Primary source: the level's selectedEntry (this runner picks a national team).
    let name = String(lvl?.selectedEntry?.name || "").trim();
    if (!name) {
        // Fallback: any player in customXi shares the team's nationality.
        const players = Array.isArray(lvl.customXi) ? lvl.customXi : [];
        for (const p of players) {
            const nat = String(p?.nationality || "").trim();
            if (nat) { name = nat; break; }
        }
    }
    if (!name) {
        // Last-ditch: currentSquad.name (the resolved team name).
        name = String(lvl?.currentSquad?.name || "").trim();
    }
    if (!name) return null;

    // Resolve a flag URL using the same convention as Runner 1.
    const flagcodes = appState.flagcodes || {};
    let flagSrc = null;
    if (name === "England") {
        flagSrc = projectAssetUrl("Images/Nationality/Europe/England.png");
    } else {
        const code = flagcodes[name];
        if (code) flagSrc = `https://flagcdn.com/w320/${String(code).toLowerCase()}.png`;
    }
    return { name, flagSrc };
}
...
function drawBanner(ctx, palette) {
    ...
    // Title text — Impact-style, two-color "GUESS THE NATIONAL TEAM".
    drawImpactTitle(
        ctx,

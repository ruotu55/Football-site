...
//   • Bottom area mirrors video mode: live background theme + perspective pitch + club logos.
//   • First quiz level player clubs on a fixed 4-3-3 formation layout.
//
...
async function drawFlagSlot(ctx, cx, cy, r, ring, imageSrc, { fit = "cover" } = {}) {
    ctx.save();
    ...
    const img = await loadFirstImage(imageSrc);
    if (img) {
        const ratio =
            fit === "contain"
                ? Math.min((r * 2) / img.width, (r * 2) / img.height)
                : Math.max((r * 2) / img.width, (r * 2) / img.height);
...
async function loadFirstImage(src) {
    const urls = Array.isArray(src) ? src : src ? [src] : [];
    for (const url of urls) {
        if (!url) continue;
        try {
            return await loadImage(url);
        } catch { /* try next crest URL */ }
    }
    return null;
}
...
async function drawVideoFormation(ctx, pitchRect) {
    ...
    for (let i = 0; i < slots.length; i++) {
        ...
        const cx = pos.x;
        const cy = pos.y;
        const logoSrc = player ? resolveClubLogoUrls(player, i) : null;
        await drawFlagSlot(ctx, cx, cy, r, ring, logoSrc, { fit: "contain" });
    }
}

function resolveClubLogoUrls(player, slotIndex) {
    ...
    if (rawOverride) {
        const overrideRel = normalizeLegacyTeamImageRelPath(
            String(rawOverride).split("?")[0].split("#")[0],
        );
        if (overrideRel) push(projectAssetUrl(overrideRel));
    }
    const clubName = player?.club || "";
    push(getClubLogoUrl(clubName));
    push(getClubLogoOtherTeamsUrl(player?.club));

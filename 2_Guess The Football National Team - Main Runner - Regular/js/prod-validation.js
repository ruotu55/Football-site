...
function getTeamHeaderFlagUrl(countryLabel, flagcodes) {
    const label = String(countryLabel || "").trim();
    if (!label) return null;
    const code = flagcodes?.[label];
    if (!code) return null;
    if (label === "England") {
 ...
 *   Players  → photo + club crest                (Team Assets)
 *   Teams    → national logo + country flag + reveal voice

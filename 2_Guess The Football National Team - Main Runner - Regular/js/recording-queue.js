...
async function onBlockClick(item) {
    const existing = blocks[item.key];
    if (existing) {
        // Mark active so Record Video uses its name. The script-applied event
        ...
        try {
            // Rebuild the script from the block's teams list (the source of truth).
            // Blocks created/edited via the calendar store only the list, not a
            // full script with levels — applying that stale/empty script is what
            // triggered "this saved script is from an older version". Fall back to
            // a stored script that already has levels.
            let script = null;
            ...
            // Carry over any frozen voice picks stashed in the stored script so
            // the voice stays stable across loads.
                ...
                if (Array.isArray(stash.levels) && Array.isArray(script.levels)) {
                    stash.levels.forEach((lvl, i) => {
                        if (lvl && lvl.voiceFreeze && script.levels[i]) {
                            script.levels[i].voiceFreeze = lvl.voiceFreeze;
                        }
                    });
                }
            }
            ...
            await applyScriptObject(script);
            // Persist any freshly-rolled voice into the stored script (top-level
            // block fields are whitelisted by the server, but block.script is kept).
            if (stash) {

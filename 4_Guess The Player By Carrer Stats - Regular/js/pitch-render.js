...
function createCareerGetPhotoControls(playerName, clubName) {
  ...
  const openSwitchModal = async () => {
    ...
    const indices = await listExistingReadyPhotoVariantIndices(playerName, careerReadyPhotoClubName(getState()));
    switchList.innerHTML = "";
    if (!indices.length) {
    ...
    const switchModalClub = careerReadyPhotoClubName(getState());
    for (const v of indices) {
      const url = await pickLoadableReadyPhotoUrlForVariant(playerName, switchModalClub, v);
      if (!url) continue;
...
async function pickLoadableReadyPhotoUrlForVariant(playerName, clubName, variantIndex) {
  ...
  const tryVariant = async (vi) => {
    for (const rel of careerReadyPhotoRelCandidates(playerName, clubName ?? "", vi)) {
...
async function readyPhotoVariantExists(playerName, clubName, variantIndex) {
  ...
  const v = Math.max(1, Math.floor(Number(variantIndex) || 1));
  for (const rel of careerReadyPhotoRelCandidates(playerName, clubName ?? "", v)) {
    const url = projectAssetUrlFresh(rel);
...
export function preloadCareerAssets(state) {
  ...
  if (playerName) {
    ...
    const readyRel = careerReadyPhotoRelPath(playerName, careerReadyPhotoClubName(state), vi);
...
export function renderCareer() {
  ...
  /* Clean stale hoisted chrome from prior renders (clear button + Get photo controls). */
  document
    .body
  ...
  document
    ...
    .forEach((el) => el.remove());
  document.getElementById("player-voice-chrome-mount")?.querySelector(".career-clear-player-btn")?.remove();
  document.getElementById("player-voice-chrome-mount")?.querySelector(".career-delete-image-btn")?.remove();
  ...
  const bindCareerReadyPhotoVariantDblClick = (el) => {
    ...
    el.addEventListener(
      ...
      async (e) => {
        ...
        const indices = await listExistingReadyPhotoVariantIndices(playerName, careerReadyPhotoClubName(st));
  ...
  if (hasRealPlayer) {
    /* Last meaningful club from career road (for Ready photo subfolder on server saves). */
    let readyPhotoClub = "";
    const histForPhoto = Array.isArray(state.careerHistory) ? state.careerHistory : [];
    for (let i = histForPhoto.length - 1; i >= 0; i--) {
      const row = histForPhoto[i];
    ...
    const getPhotoUi = createCareerGetPhotoControls(playerName, readyPhotoClub);
    ...
    if (voiceChromeMountEarly) {
      ...
      if (getPhotoUi) wrap.appendChild(getPhotoUi.host);
    }
  ...
  revealFallback.textContent = hasRealPlayer
    ...
    for (let ri = 0; ri < clubSlotRows.length; ri++) {
      ...
      if (rowIndices.length === 1) {
        rowEl.classList.add("player-stat-clubs-row--single");
      }

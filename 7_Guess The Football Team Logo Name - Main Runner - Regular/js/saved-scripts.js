// js/saved-scripts.js � Main Runner - Team Logo Name - Regular (storage isolated from other runners)
import {
    appState,
...
const VOICE_FREEZE_MIGRATION_FLAG = "footballQuizVoiceFreezeMigrated_team_logo_name_regular_v2";
...
const KEY_SCRIPTS = "footballQuizScripts_team_logo_name_regular_v1";
const KEY_FOLDERS = "footballQuizFolders_team_logo_name_regular_v1";
const KEY_FOLDER_STATES = "footballQuizFolderStates_team_logo_name_regular_v1";
...
const SAVE_SERVER = createSavedScriptsServerSync("team_logo_name_regular", {
    KEY_SCRIPTS,
    KEY_FOLDERS,
    KEY_FOLDER_STATES,
});
 ...
 *  commitSavedScript() captures below � keep in sync if that helper changes. */
...
/** Build a script object from a "[Team1, Team2, ...]" (or player list) paste �
 *  the same flow the legacy Import modal ran, but headless so the calendar-
 *  driven Saved tab can call it from its block-save modal. Returns one of:
 *    { ok: true, script }
 *    { ok: false, errors: string[], searchableNames: Set<string> }  // resolution needed
 *    { ok: false, errors: ["..."] }                                   // parse failure
 *  Does NOT mutate UI state (no activeScriptName, no rendering). Ambiguous
 *  names auto-resolve to the first candidate (headless � no disambig modal). */
...
function stripStandaloneNumbersForClubMatch(norm) {
    return String(norm || "")
        .replace(/\b\d+\b/g, " ")
        .replace(/\s+/g, " ")
        .trim();
...
/** Same shape as `createCareerTeamFromEntry` in app.js (team-name / fake-info levels). */
 ...
 * Mirrors player import disambiguation when multiple clubs match.
 */
...
function findAllClubCandidates(rawName, clubs) {
    ...
    const queryNoNums = stripStandaloneNumbersForClubMatch(query);

    if (/^rb\s+/i.test(raw)) {
        query = clubKeyForMatch(raw.replace(/^\s*rb\s+/i, "Red Bull "));
    }

    let hits = clubs.filter((c) => {
        const k = clubKeyForMatch(c.name);
        if (!k) return false;
        if (k === query) return true;
        if (queryNoNums && stripStandaloneNumbersForClubMatch(k) === queryNoNums) return true;
        return false;
    });
    ...
    hits = clubs.filter((c) => {
        ...
        if (query.length >= 6 && k.length >= 6 && query.includes(k)) return true;
        return false;
    });
...
export function initSavedScripts(callbacks) {
        ...
        return new Promise((resolve) => {
            ...
            for (let i = 0; i < ambig.length; i++) {
                ...
                for (let ci = 0; ci < candidates.length; ci++) {
                    ...
                    opt.value = String(ci);
                    const isTeamIndexEntry =
                        !!(p && typeof p.path === "string" && typeof p.league === "string");
                    opt.textContent = isTeamIndexEntry
                        ? `${p.name} � ${p.country || "?"} / ${p.league}`
                        : `${p.name} (${(p?._clubItem?.name) || p?.club || "?"})`;
    ...
    if (els.importScriptConfirm) {
        ...
        parsed.entries ? teamNamesFromPairEntries(parsed.entries) : parsed.names,
                ...
                const ambiguous = [];
                let disambigItems = [];
                let disambigDisplayFn = (x) => String(x);
                let disambigModalTitle = "Pick a match";
                ...
                if (importTeams) {
                    ...
                    disambigItems = allPlayers;
                    disambigDisplayFn = (p) => `${p.name} - ${p?._clubItem?.name || "?"}`;
                    disambigModalTitle = "Search a player";
                    for (let i = 0; i < names.length; i++) {
                        const rawName = names[i];
                        const cands = findAllPlayerCandidates(rawName, allPlayers);
                        if (cands.length === 0) {
                ...
                if (errors.length > 0) {
                    renderImportErrors({
                        ...
                        searchableNames,
                        items: disambigItems,
                        displayFn: disambigDisplayFn,
                        modalTitle: disambigModalTitle,
...
async function loadScript(script) {
    ...
    appState.levelsData.forEach((merged, i) => {
        restoreLevelVoiceFreeze(merged, script.levels[i]?.voiceFreeze);
    });

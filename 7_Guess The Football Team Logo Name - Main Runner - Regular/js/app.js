...
import { switchLevel, syncFloatingShirtVisibilityFromLevel } from "./levels.js";
import {
    renderHeader,
    renderCareer,
    cleanCareerHistory,
...
const QUIZ_TYPE_DEFAULT_THEME = {
    "player-by-fake-info": { colorId: "extra-soft-green", effectId: "center-rings", opacity: 5, transitionId: "n2-8" },
};
...
export function updateSetupUI() {
    ...
    if (els.setupCareerClubsField) els.setupCareerClubsField.style.display = "none";
...
export function populateSubTypes() {
    ...
    els.inQuizType.innerHTML = `
      <option value="player-by-fake-info">GUESS THE FOOTBALL TEAM NAME</option>
    `;
...
export function updateLanding() {
    ...
    document.getElementById("landing-special-text").textContent = getSpecificTitleText(els.inSpecificTitlePreset?.value || DEFAULT_SPECIFIC_TITLE_PRESET_KEY, getCurrentLanguage());
    ...
    const useSpecificTitleImage =
        (iconVal.startsWith("icons/") || iconVal.startsWith("Images/")) || iconVal.startsWith("Images/");
...
async function init() {
    ...
    els.videoModeToggle.onchange = (e) => {
        ...
        const quizTypeLabel = String(els.inQuizType?.selectedOptions?.[0]?.textContent || "")
            .trim()
            .toUpperCase();
        const teamYearMarker = String(
            state.careerHistory?.[0]?.year ?? state.careerPlayer?.transfer_history?.[0]?.year ?? ""
        ).trim().toUpperCase();
        const hasSelectedTeamCard =
            !!state.careerPlayer &&
            (
        ...
        if (isQuestionLevel || hasSelectedTeamCard) {
            renderCareer();
        }
        renderHeader();
        renderLandingTitleVoiceControls();

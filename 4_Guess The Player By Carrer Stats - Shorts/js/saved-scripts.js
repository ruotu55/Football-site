...
const VOICE_FREEZE_MIGRATION_FLAG = "footballQuizVoiceFreezeMigrated_placholder2_shorts_v2";
 ...
 *  the Import modal ran, but headless so the calendar-driven Saved tab can call
...
export async function buildScriptFromImportText(text, name) {
    ...
    for (let i = 0; i < names.length; i++) {
        ...
        if (cands.length === 0) {
            ...
            // Headless build can't show a disambiguation picker — surface it so
            // the user resolves it via the regular Import flow first.
            errors.push(`❌ ${rawName}: multiple players match (resolve via Import).`);
            searchableNames.add(rawName);
        }
    }

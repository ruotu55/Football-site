...
/** Question bar + timeouts: `baseSteps` equal slices of this total (5 × 1s × 1.0 = 5s). */
const SHORTS_QUESTION_COUNTDOWN_DURATION_MULT = 1.0;
...
const SHORTS_INTRO_QUIZ_TITLE_LINE_1 = "GUESS THE PLAYER BY CLUB";
const SHORTS_INTRO_QUIZ_TITLE_LINE_2 = "POSITION COUNTRY AGE";
const SHORTS_INTRO_QUIZ_TITLE_LINE_1_ES = "ADIVINA EL JUGADOR POR CLUB,";
const SHORTS_INTRO_QUIZ_TITLE_LINE_2_ES = "POSICIÓN, PAÍS Y EDAD";
...
function revealCurrentLevel() {
  ...
  if (
    ...
    if (!skipBonusReveal) {
      ...
      /* Avoid `refreshCurrentQuestionPreview()` here — it calls `renderCareer()`
         which does `wrap.innerHTML = ""` and rebuilds every IMG (club crest,
         flag, etc.) for one frame, causing the visible "blink/disappear-reappear"
         on reveal. The dim removal on the silhouette is already CSS-driven via
         the `shorts-question-countdown` class being removed on the body, so the
         only DOM update we actually need on reveal is the mystery-mark text
         swap from "?" → player name. The minimal helper does just that. */
      applyMinimalVideoRevealUpdate();
      flipDelay = 3000;
    } else {

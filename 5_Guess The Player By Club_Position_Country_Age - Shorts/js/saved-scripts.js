...
const VOICE_FREEZE_MIGRATION_FLAG = "footballQuizVoiceFreezeMigrated_four_parameters_shorts_v2";
...
const KEY_SCRIPTS = "footballQuizScripts_four_parameters_shorts_v1";
const KEY_FOLDERS = "footballQuizFolders_four_parameters_shorts_v1";
const KEY_FOLDER_STATES = "footballQuizFolderStates_four_parameters_shorts_v1";
...
const SAVE_SERVER = createSavedScriptsServerSync("four_parameters_shorts", {
    KEY_SCRIPTS,
    KEY_FOLDERS,
    KEY_FOLDER_STATES,
});
 ...
 *  Players that resolve to more than one candidate are treated as unresolved
 *  here (no interactive disambiguation modal in the headless path). Does NOT
 *  mutate UI state (no activeScriptName, no rendering). */
    ...
    if (!pairResolved) {
        ...
        for (let i = 0; i < names.length; i++) {
            ...
            if (cands.length === 0) {
                ...
                errors.push(`? ${rawName}: multiple players match � use the Import tab to pick one.`);
                searchableNames.add(rawName);
            }
        }

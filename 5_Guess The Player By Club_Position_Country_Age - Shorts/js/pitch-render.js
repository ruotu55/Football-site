...
/* Smooth fade swap between the "?" placeholder and the revealed player name.
   Uses inline CSS transitions on opacity (more predictable than the Web
   Animations API for this case ג€” no compositing surprises with the existing
   transform/font-size rules on the mark). A monotonic token guards against
   rapid back-to-back swaps so a stale callback can't clobber a newer one. */
...
function fadeSwapMysteryText(el, newText, isName) {
  ...
  setTimeout(() => {
    ...
    /* Phase 3: fade the new content in.
       ...
       The fade-out's transition rule (`opacity 220ms ease-in`) is still in the
       computed style cascade. If we just overwrite `transition` + flip opacity,
       the browser may coalesce both writes into a single style update and the
       fade-in won't actually animate (instant pop-in ג€” what was happening).

       The reliable pattern: kill the transition with `none`, force a reflow
       (`offsetWidth` read commits pending styles), then set the new transition
       rule, then flip opacity inside rAF so the new rule + opacity:0 baseline
       are committed before the value change triggers it. */
...
/* Minimal post-timer reveal update. Used in place of `renderCareer()` on reveal
   so the wrap isn't blown away ג€” destroying every <img> would otherwise cause
   the club crest, flag, and other cached images to blink for a frame as their
   render layers are recreated. Only the mystery-mark text needs to swap from
   "?" to the player name; the dim-removal on the silhouette is CSS-driven via
   the `shorts-question-countdown` class being removed on the body, so no JS
   needed for that. */
export function applyMinimalVideoRevealUpdate() {
  const wrap = document.getElementById("career-wrap");
  if (!wrap) return;
  const state = getState();
  if (!state) return;
  const portraitCardVm = wrap.querySelector(".career-portrait-card");
  if (!portraitCardVm) return;
...
/* Auto-shrink the revealed name only when the rendered text actually overflows
   ...
   No-op when the text fits ג€” preserves the larger size for short names. */
function autoFitMysteryName(el) {
  if (!el) return;
  // Clear any prior override so the base CSS clamp re-applies first.
  el.style.fontSize = "";
  // Defer measurement until the browser has laid out the freshly-set textContent.
  requestAnimationFrame(() => {
    ...
    let guard = 60; // hard cap on iterations
    while (
      guard-- > 0 &&
      size > floor &&
...
export function applyCareerPictureModeToActiveState(st, isShortsLayout) {
  if (!st) return;
  if (isShortsLayout) {
    /* Shorts layout: unify VM On/Off/Play Video ג€” one Adjust Picture ג€” Shorts profile that
...
export function persistCareerPictureModeFromActiveState(st, isShortsLayout) {
  ...
  if (isShortsLayout) {
    /* Shorts layout: one unified profile ג€” write to both VM On + VM Off shorts fields so a
...
export function renderCareer() {
  ...
  if (!FOUR_PARAMS_HIDE_INLINE_PLAYER_PICKER && !hasRealPlayer && !shortsPreviewActive && !state.isOutro) {
    ...
    /* Mount on <body> so no transformed/filtered ancestor interferes with the
       `position: fixed` centering ג€” matches the Lineups Shorts team picker. */
  ...
  if (hasRealPlayer) {
    ...
    if (isShorts && voiceChromeMount) {
      voiceChromeMount.insertBefore(deleteImageBtn, clearBtn);
    } else {
  ...
  const buildRealValueInner = (stat, player) => {
    ...
    if (stat === "club") {
      ...
      /* Sync decoding so the cached crest paints in the same frame the IMG is
         attached ג€” prevents the "blink" on reveal where async-decoded cached
         images leave a 1-frame blank as the wrap is rebuilt. */
  ...
  const mountFourParamCareerCards = (gridEl, player) => {
    ...
    logoImg.loading = "eager";
    /* Sync decoding so the cached crest paints in the same frame the IMG is
       attached, preventing the "blink/disappear-reappear" on reveal when the
       wrap is rebuilt and the new IMG would otherwise paint blank for one
       frame while async-decoding from cache. */
    logoImg.decoding = "sync";
...
/* ג”€ג”€ Shorts-only helpers ported from Career Path Shorts ג”€ג”€ג”€ג”€ג”€ג”€ג”€ג”€ג”€ג”€ */

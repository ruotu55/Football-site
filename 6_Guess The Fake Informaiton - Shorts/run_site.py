...
# Per-runner voice isolation for clips that SHOULD differ between runners.
# Scope intentionally narrow:
#   • Quiz Intro  → per runner (each main runner has its own intro line)
#   • Quiz Sounds → per runner (e.g. fake-stats clips only apply to fake-info mode)
#   • Players     → SHARED across runners AND across languages (single name pool,
#                   regardless of which language was used to synthesise it)
#   • Endings     → SHARED across runners, split per language
# Every other clip is also stored under a LANGUAGE subdirectory (english / spanish),
# so switching languages in the Voice tab doesn't clobber the other language's pool.
# To port this to another runner, change RUNNER_VARIANT and the per-runner prompts below.
RUNNER_VARIANT = "Four Params Shorts"
...
# Phrase variants applied to the reveal voice. "plain" = just the name; the rest
# are full sentences that bake the answer name into a single TTS clip. The set is
# kind-aware: `team-is` is only valid when the answer is a team, `player-is` when
# the answer is a player; both keys are accepted here so saved clips of either
# variant resolve, and the voice tab decides which subset to expose per-kind.
TEAM_PHRASE_KEYS = (
    "plain",
    "correct-answer",
    "right-answer",
...
QUIZ_TITLE_PROMPT_BY_QUIZ_TYPE = {
    "english": {
        ...
        "player-by-fake-info": "GUESS THE FAKE STAT ABOUT THE PLAYER",
    },
    "spanish": {
...
# Bundled voices (Level progress). Both languages are stored under a
# `<lang>/` subfolder of the base dir, synthesised on-demand via ElevenLabs with the
# prompt below. Shared across all runners (voice tab is the source of truth).
BUNDLED_VOICE_CONFIG = {
    "warm-up": {
        "dir": PROJECT_ROOT / ".Storage" / "Voices" / "Levels",
        "filename": "Worm up round dont mess this one .mp3",
        "prompts": {
            "english": "Warm up round — don't mess this one!",
    ...
    "nerds": {
        ...
        "prompts": {
            "english": "Only true football nerds know this!",
    ...
    "genius": {
        ...
        "prompts": {
            "english": "If you get this you are basically a genius!",
...
def _normalize_bundled_voice_inputs(key: str | None, language: str | None) -> tuple[str, str, Path]:
    k = str(key or "").strip()
    if k not in BUNDLED_VOICE_CONFIG:
        raise ValueError("Unsupported bundled voice key.")
    lang = _normalize_language(language)
    cfg = BUNDLED_VOICE_CONFIG[k]
    # Each language lives under `<dir>/<lang>/<filename>`; both languages are generated
    # on-demand via ElevenLabs, so the Voice tab can generate and delete either one.
...
def _player_voice_paths_for_name(
    ...
    """Resolve all candidate on-disk paths for one (name, kind, phrase, language) reveal clip.
    The new layout is `<base>/<lang>/<phrase>/<Name>.<ext>` where base is the kind's voice
    folder; the legacy flat layout `<base>/<Name>.<ext>` is handled separately so it only
    resolves for English plain (backward-compat lookup)."""
    base = _voice_dir_for_kind(kind)
    lang = _normalize_language(language)
    ph = str(phrase or "plain").strip() or "plain"
    if ph not in TEAM_PHRASE_KEYS:
        ph = "plain"
    target_dir = base / lang / ph
    return [target_dir / f"{player_name}{ext}" for ext in PLAYER_VOICE_ALLOWED_EXTS]


def _player_voice_legacy_root_paths(player_name: str, kind: str | None) -> list[Path]:
    """Old layout pre-phrase/language split — files lived directly under the kind's voice
    folder. Kept for backward-compat lookup so existing English plain clips still resolve."""
    base = _voice_dir_for_kind(kind)
...
def _normalize_team_voice_inputs(
    ...
    """Resolve the on-disk target directory for one (name, kind, phrase, language) reveal clip.
    The first argument after the name is `kind` (the runner-specific concept that picks
    which root folder — Players Names vs Team names — the clip lives under), preserved
    from the existing endpoint payload shape used by this runner."""
...
def _team_voice_prompt(name: str, phrase: str, language: str) -> str:
    """Render the reveal sentence and dial up the excitement: single exclamation at the
    end (ElevenLabs treats it as an emotional cue), plus Spanish inverted '¡' opener
    when needed for non-plain sentences."""
    templates = TEAM_PHRASE_TEMPLATES.get(language) or TEAM_PHRASE_TEMPLATES["english"]
    template = templates.get(phrase) or "{name}"
    rendered = template.replace("{name}", name).strip()
...
# Excited delivery for the player/team-name reveal: lower stability = more pitch variation,
# style > 0 ramps emotional range, speaker_boost tightens the timbre. Quiz-intro and
# ending voices keep the default flatter delivery (no voice_settings override).
...
def _generate_elevenlabs_speech_mp3(
    ...
    # Force the multilingual model to speak in the requested language — otherwise
    # it auto-detects from the text, which defaults to English for bare names like
    # "James Maddison" even when the rest of the tab is in Spanish.
...
class RunnerRequestHandler(SimpleHTTPRequestHandler):
    ...
    def _try_serve_player_voice_status(self) -> bool:
        ...
        try:
            player_name, target_dir, phrase, language = _normalize_team_voice_inputs(
                query.get("name"),
                query.get("kind"),
        ...
        for ext in PLAYER_VOICE_ALLOWED_EXTS:
            file_path = target_dir / f"{player_name}{ext}"
        ...
        if existing_path is None and language == "english" and phrase == "plain":
            for file_path in _player_voice_legacy_root_paths(player_name, query.get("kind")):
                if file_path.is_file():
                    existing_path = file_path
                    break
        self._send_json(
    ...
    def _try_generate_player_voice(self) -> bool:
        ...
        for ext in PLAYER_VOICE_ALLOWED_EXTS:
            old_path = target_dir / f"{player_name}{ext}"
        ...
        provider = "elevenlabs"
        prompt_text = _team_voice_prompt(player_name, phrase, language)
        try:
    ...
    def _try_delete_player_voice(self) -> bool:
        ...
        if language == "english" and phrase == "plain":
            for file_path in _player_voice_legacy_root_paths(player_name, body.get("kind")):
                if file_path.exists():
                    file_path.unlink(missing_ok=True)
                    removed += 1

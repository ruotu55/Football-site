...
const SHORTS_INTRO_QUIZ_TITLE_LINE_1_ES = "ENCUENTRA EL DATO FALSO";
const SHORTS_INTRO_QUIZ_TITLE_LINE_2_ES = "SOBRE EL JUGADOR";
...
function clearShortsQuestionCountdown() {
  // Fake-info quiz: keep .shorts-question-countdown on <body> so the dozens of
  // rules conditional on it don't all toggle off in the same frame and flash the
  // team logo / crests / photo. The next countdown re-adds the same class — a no-op.
  // Other call sites (stop video, end of flow) still strip it via their own paths.
  if (!document.body.classList.contains("fake-info-quiz")) {
...
function revealCurrentLevel() {
  ...
  if (
    ...
    if (!skipBonusReveal) {
      ...
      // Skipping the DOM rebuild prevents the player photo / team-logo flash caused by
      // wrap.innerHTML="" replacing every <img> element on timer-end.
      if (!isFakeInfoQuiz()) {
        refreshCurrentQuestionPreview();
      }
      flipDelay = 3000;

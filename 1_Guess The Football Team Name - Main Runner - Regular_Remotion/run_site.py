...
def _fetch_bytes(url: str, *, timeout: float = 35.0) -> bytes:
    # Image CDNs (UEFA, Cloudinary-fronted img.fcbayern.com, ...) STALL when the
    # request advertises image/avif (content-negotiation hang) or points Referer
    # at the image's own host. Use a plausible browser recipe + per-host Referer.
...
def _team_voice_legacy_root_paths(team_name: str, quiz_type: str) -> list[Path]:
    ...
    q = quiz_type if quiz_type in TEAM_VOICE_DIR_BY_QUIZ_TYPE else "club-by-nat"
...
class RunnerRequestHandler(SimpleHTTPRequestHandler):
    ...
    def _try_serve_team_voice_status(self) -> bool:
        ...
        if existing_path is None and language == "english" and phrase == "plain":
            for file_path in _team_voice_legacy_root_paths(
                team_name, str(query.get("quizType") or "club-by-nat")
            ):
                if file_path.is_file():
                    existing_path = file_path
                    break
    ...
    def _try_delete_team_voice(self) -> bool:
        ...
        if language == "english" and phrase == "plain":
            for file_path in _team_voice_legacy_root_paths(
                team_name, str(body.get("quizType") or "club-by-nat")
            ):
                if file_path.exists():
                    file_path.unlink(missing_ok=True)
                    removed += 1
    ...
    def _try_rename_team_voice(self) -> bool:
        ...
        try:
            ...
            if not _is_valid_windows_filename_stem(old_raw) or not _is_valid_windows_filename_stem(new_raw):
                raise ValueError("Team name has unsupported filename characters for Windows.")
            quiz_type = str(body.get("quizType") or "club-by-nat").strip()
            if quiz_type not in TEAM_VOICE_DIR_BY_QUIZ_TYPE:
                quiz_type = "club-by-nat"
        except ValueError as exc:
            self._write_json(400, {"ok": False, "error": str(exc)})
            return True
        if old_raw == new_raw:

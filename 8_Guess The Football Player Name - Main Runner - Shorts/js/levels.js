...
export function switchLevel(index) {
  ...
  if (isQuestionLevel && !state.careerPlayer) {
    ...
    if (baselineQuestionState && baselineQuestionState !== state) {
      ...
      /* Each level starts with fresh default picture settings (0 / 1.0 / 1.0). */
  ...
  const updateDOMContent = () => {
    /* Clear reveal body classes before rendering the next level's DOM — otherwise
       newly-mounted cards inherit the fake-info / post-timer animation and appear in
       the revealed (forward + blurred) state from frame one. Mirrors Regular levels.js:527-528. */

...
registry.set("n2-3", {
  ...
  run: _newRun("n2-3-overlay",
    ...
    (gsap, c) => {
      ...
      const back = c.querySelector(".n2-3-back");
      return _tl(gsap, t => {
        // Cells shrink first; back stays opaque so the gappy diamond grid never exposes the new content.
        t.to(cells, { duration: N2_DUR * 0.4, scale: 0, stagger: { amount: N2_DUR * 0.25, grid: [6, 11], from: "random" }, ease: "power2.in" }, 0);
        // Back fades only after the cells are mostly gone, then over the remaining slot.
        t.to(back, { duration: N2_DUR * 0.3, opacity: 0, ease: "power1.out" }, N2_DUR * 0.55);
      });
    }
  ),
});

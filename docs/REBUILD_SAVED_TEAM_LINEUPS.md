...
4b. **Only genuine first-team players — NEVER a player from another club.** Some
   squad JSONs include B-team / reserve / loan entries whose `club` field is a
   *different* club (e.g. "Miguel Sierra" sits in `Sevilla FC.json` but his club
   is "Sevilla Atlético"; Real Madrid's file contains Castilla players; Barça's
   contains Barça Atlètic, etc.). These must **never** appear in an XI — on the
   pitch their card shows the wrong club name. This is now enforced two ways
   (see §2): `dump_rosters.py` **excludes** them from what agents see, and
   `apply.py` **rejects** any pick whose `club` != the team's own name.
...
- **Cross-club / B-team players (the "player from another team" bug):** squad
  JSONs can list reserve/loan players whose `club` is a different club. If one is
  put in an XI, the pitch card shows that other club's name. `dump_rosters.py`
  now drops these from the agent-facing roster files (and logs them to stderr),
  and `apply.py` hard-rejects any assignment containing one (`CROSS-CLUB
  PLAYER ... belongs to X, not Y`). To find them in the *current* blob, compare
  each `customXi` player's `club` to the squad file's `name`.

...
/* Smooth fade swap between the "?" placeholder and the revealed player name.
   ...
   this case ג€” no compositing surprises with the existing transform/font-size
   rules on the mark). Monotonic token guards against rapid back-to-back swaps
   so a stale callback can't clobber a newer one. The reflow flush in phase 3
   is required: without it the browser coalesces the transition rule update +
   opacity flip into a single style write and the fade-in won't animate. */
...
function fadeSwapMysteryText(el, newText, isName) {
  ...
  setTimeout(() => {
    ...
    /* Phase 3: fade-in. Must clear transition + force reflow first or the
       browser will coalesce the rule update + opacity flip and skip the
       transition (instant pop-in). */
...
/* Auto-shrink the revealed name only when the rendered text actually overflows
   the mystery-mark box. The CSS applies `font-size: clamp(...)` which is fine
   for most names; long ones can still overflow even at the clamp's minimum, so
   we step the size down at runtime past the CSS floor. No-op when text fits. */

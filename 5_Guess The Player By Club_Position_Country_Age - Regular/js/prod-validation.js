...
export function collectPlayerCardUnits(lvl) {
    const player = lvl.careerPlayer;
    const name = String(player?.name || "").trim();
    if (!name) return [{ label: "player photo", urls: [] }];

    const units = [];
    // Player photo — ready-photo candidates for the chosen variant, then variant 1
    // (mirrors pickLoadableReadyPhotoUrlForVariant's fallback).
    ...
    // Country flag tile (England = repo asset, else flagcdn). Skip when no flag is
    // shown (unresolved nationality) — the card hides it too, so there's nothing to check.

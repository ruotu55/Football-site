...
// ("…let's get started. Guess the national team…"). The ball opens at the pause
...
const GREETING_PORTION_SEC: Record<string, number> = { english: 3.80, spanish: 3.96 };
// Where the spoken line ends in the combined clip (trailing silence start, measured).
const SPEECH_END_SEC: Record<string, number> = { english: 6.77, spanish: 7.32 };
...
const introTitleFrames = (language: DemoProps["language"], transFrames: number): number =>
  USE_ULTIMATE_INTRO
    ? Math.max(40, Math.ceil((SPEECH_END_SEC[langKey(language)] ?? 6.77) * DESIGN_FPS) - greetingPortionFrames(language) + transFrames + TITLE_TAIL_FRAMES)

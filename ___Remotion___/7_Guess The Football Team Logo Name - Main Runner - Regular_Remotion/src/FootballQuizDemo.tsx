...
// Intro toggle: the new branded "Ultimate Football Quiz" channel intro vs the original
// 4-ball merge. Set false to revert to BallIntro.
...
// ("…let's get started. Guess the football team logo name."). The ball opens at the pause
// BETWEEN them so the title is revealed as "Guess the team…" is said.
// GREETING_PORTION_SEC = that pause point per language (measured from the clip).
const GREETING_START_F = 4;
const GREETING_PORTION_SEC: Record<string, number> = { english: 3.60, spanish: 3.93 };
// Where the spoken line ends in the combined clip — the quiz-title screen ends + level 1
// transitions in right here.
const SPEECH_END_SEC: Record<string, number> = { english: 5.81, spanish: 6.54 };
const greetingPortionFrames = (language: DemoProps["language"]) =>
  Math.ceil((GREETING_PORTION_SEC[langKey(language)] ?? 3.60) * DESIGN_FPS);
...
// the spoken "Guess the team…" part (so level 1 transitions in exactly when the voice ends).
...
const introTitleFrames = (language: DemoProps["language"], transFrames: number): number =>
  USE_ULTIMATE_INTRO
    ? Math.max(40, Math.ceil((SPEECH_END_SEC[langKey(language)] ?? 5.81) * DESIGN_FPS) - greetingPortionFrames(language) + transFrames + TITLE_TAIL_FRAMES)

...
// Intro toggle (runner 3 only): the new branded "Ultimate Football Quiz" channel
// intro vs the original 4-ball merge. Same duration (BALL_INTRO_FRAMES) so flipping
// this is a clean swap with no timing changes — set false to revert to BallIntro.
...
// ("…let's get started. Guess the player by career path."). The ball opens at the pause
// BETWEEN them (~4.05s into the clip) so the title is revealed as "Guess the player…" is
// said. GREETING_PORTION_SEC = that pause point per language (measured from the clip).
const GREETING_START_F = 4;
const GREETING_PORTION_SEC: Record<string, number> = { english: 4.05, spanish: 4.05 };
// Where the spoken line ends in the combined clip (trailing silence start, measured) — the
// quiz-title screen ends + level 1 transitions in right here.
const SPEECH_END_SEC: Record<string, number> = { english: 6.27, spanish: 6.64 };
const greetingPortionFrames = (language: DemoProps["language"]) =>
  Math.ceil((GREETING_PORTION_SEC[langKey(language)] ?? 4.05) * DESIGN_FPS);
...
// spoken "Guess the player…" part (so level 1 transitions in exactly when the voice ends).
...
const introTitleFrames = (language: DemoProps["language"], transFrames: number): number =>
  USE_ULTIMATE_INTRO
    ? Math.max(40, Math.ceil((SPEECH_END_SEC[langKey(language)] ?? 6.3) * DESIGN_FPS) - greetingPortionFrames(language) + transFrames + TITLE_TAIL_FRAMES)

...
// ("…let's get started. Guess the fake information."). The ball opens at the pause
// BETWEEN them so the title is revealed as "Guess the fake…" is said.
// GREETING_PORTION_SEC = that pause point per language (measured from the clip).
const GREETING_START_F = 4;
const GREETING_PORTION_SEC: Record<string, number> = { english: 3.70, spanish: 3.80 };
// Where the spoken line ends in the combined clip (trailing silence start, measured).
const SPEECH_END_SEC: Record<string, number> = { english: 6.21, spanish: 6.40 };
const greetingPortionFrames = (language: DemoProps["language"]) =>
  Math.ceil((GREETING_PORTION_SEC[langKey(language)] ?? 3.70) * DESIGN_FPS);
...
// spoken "Guess the fake…" part (so level 1 transitions in exactly when the voice ends).
...
const introTitleFrames = (language: DemoProps["language"], transFrames: number): number =>
  USE_ULTIMATE_INTRO
    ? Math.max(40, Math.ceil((SPEECH_END_SEC[langKey(language)] ?? 6.21) * DESIGN_FPS) - greetingPortionFrames(language) + transFrames + TITLE_TAIL_FRAMES)

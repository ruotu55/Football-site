...
// ("…let's get started. Guess the player by career stats."). The ball opens at the pause
// BETWEEN them (~3.80s into the EN clip) so the title is revealed as "Guess the player…"
// is said. GREETING_PORTION_SEC = that pause point per language (measured from the clip).
const GREETING_START_F = 4;
const GREETING_PORTION_SEC: Record<string, number> = { english: 3.80, spanish: 3.94 };
...
const SPEECH_END_SEC: Record<string, number> = { english: 6.32, spanish: 7.06 };
const greetingPortionFrames = (language: DemoProps["language"]) =>
  Math.ceil((GREETING_PORTION_SEC[langKey(language)] ?? 3.80) * DESIGN_FPS);
...
const introTitleFrames = (language: DemoProps["language"], transFrames: number): number =>
  USE_ULTIMATE_INTRO
    ? Math.max(40, Math.ceil((SPEECH_END_SEC[langKey(language)] ?? 6.32) * DESIGN_FPS) - greetingPortionFrames(language) + transFrames + TITLE_TAIL_FRAMES)

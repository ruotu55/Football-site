...
// original 4-ball merge. Same IRIS_FRAMES overlap so flipping this is a clean swap
// with no timing changes — set false to revert to BallIntro.
...
// ("…let's get started. Guess the football player name."). The ball opens at the pause
// BETWEEN them so the title is revealed as "Guess the football player…" is said.
// GREETING_PORTION_SEC = that pause point per language (measured from the clip).
const GREETING_START_F = 4;
const GREETING_PORTION_SEC: Record<string, number> = { english: 3.82, spanish: 3.88 };
// Where the spoken line ends in the combined clip (trailing silence start) — the
// quiz-title screen ends + level 1 transitions in right here.
const SPEECH_END_SEC: Record<string, number> = { english: 5.95, spanish: 6.51 };
const greetingPortionFrames = (language: DemoProps["language"]) =>
  Math.ceil((GREETING_PORTION_SEC[langKey(language)] ?? 3.82) * DESIGN_FPS);
...
// the spoken "Guess the football player name" part (so level 1 transitions in exactly
// when the voice ends).
...
const introTitleFrames = (language: DemoProps["language"], transFrames: number): number =>
  USE_ULTIMATE_INTRO
    ? Math.max(40, Math.ceil((SPEECH_END_SEC[langKey(language)] ?? 5.95) * DESIGN_FPS) - greetingPortionFrames(language) + transFrames + TITLE_TAIL_FRAMES)

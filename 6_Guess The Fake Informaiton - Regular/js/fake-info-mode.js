...
export function fakeInfoPickForLevel(state, levelIndex) {
  ...
  /* Avoid the same fake stat as the immediately previous question level so two
     consecutive levels can't share the same fake (e.g. position → position).
     Only the backward direction is enforced — level N+1 reading this pick handles
     forward avoidance, so the constraint propagates one-way through the chain. */
  let prevStat = null;
  if (levelIndex > 0) {
    const prevState = appState?.levelsData?.[levelIndex - 1];
    if (prevState?.careerPlayer?.name) {
      prevStat = fakeInfoPickForLevel(prevState, levelIndex - 1)?.stat || null;
    }
  }

  /* Try stats in a randomized order; skip any that can't produce a fake for this player. */
  let order = [...FAKE_STAT_KEYS].sort(() => rand() - 0.5);
  if (prevStat && order.includes(prevStat)) {
    /* Push the previous level's stat to the back: it's used only as a last-resort
       fallback if no other stat can produce a fake for this particular player. */
    order = order.filter(s => s !== prevStat).concat(prevStat);
  }
...
function computeFakeForStat(stat, player, rand) {
  ...
  if (stat === "position") {
    ...
    /* Never fake a goalkeeper's position (too obvious), and never fake a regular
       player's position to goalkeeper. Skip the position stat in either case so
       the picker falls through to a different stat. */
    if (currentCategory === "goalkeeper") return null;
    const categories = Object.keys(POSITION_BY_CATEGORY).filter(
      (c) => c !== currentCategory && c !== "goalkeeper"
    );

/* Voice tab — Career Path Regular. Mirrors the Four Params Shorts implementation,
...
const QUIZ_TYPE_PROMPTS = {
  english: {
    "player-by-career":         "Hey everyone, let's start. ... GUESS THE PLAYER BY CAREER PATH!!",
    "player-by-career-stats":   "Hey everyone, let's start. ... GUESS THE PLAYER BY CAREER PATH!!",
  },
  spanish: {
    "player-by-career":         "Hola a todos, empecemos. ... ¡¡ADIVINA AL JUGADOR POR TRAYECTORIA!!",
    "player-by-career-stats":   "Hola a todos, empecemos. ... ¡¡ADIVINA AL JUGADOR POR TRAYECTORIA!!",
  },
};
...
function bundledMilestonePlaysAt(milestone) {
  const lang = getCurrentLanguage();
  return milestone.playsAt[lang] || milestone.playsAt.english;
}
...
function buildRow({ text, exists, onPlay, onDelete, playsAt = "", deleteDisabled = null, sessionPick = false }) {
  ...
  vol.type = "button"; vol.className = "voice-tab-btn voice-tab-btn--vol"; vol.textContent = "Vol";
  vol.title = exists ? "Play" : "Generate and play";
  vol.onclick = (e) => { e.preventDefault(); onPlay(); };
  const del = document.createElement("button");
  del.type = "button"; del.className = "voice-tab-btn voice-tab-btn--x"; del.textContent = "X";
  ...
  const main = document.createElement("span"); main.className = "voice-tab-row__text-main"; main.textContent = text;
  ...
  row.appendChild(vol); row.appendChild(del); row.appendChild(wrap);
  return row;
}
...
function buildSection(title, rows) {
  ...
  const summary = document.createElement("summary"); summary.className = "voice-tab-section__title"; summary.textContent = title;
  wrap.appendChild(summary);
  if (rows.length === 0) {
    const empty = document.createElement("div"); empty.className = "voice-tab-section__empty"; empty.textContent = "— none";
...
export async function renderVoiceTab() {
  ...
  if (quizType) {
    quizRows.push(buildRow({
      ...
      onPlay: () => onVolPressed({
        ...
        cachedExists: quizTitleStatus.exists, cachedSrc: quizTitleStatus.src,
  ...
  const endingRows = [
    ...
    .map(({ type, status }) => buildRow({
      text: endingTextMap[type], exists: status.exists,
      playsAt: plays("ending"),
      onPlay: () => onVolPressed({
        rowKey: `ending:${type}:${lang}`,
        cachedExists: status.exists, cachedSrc: status.src,
        generateEndpoint: "__ending-voice/generate", generateBody: { endingType: type },
      }),
      onDelete: () => onDeletePressed({
        rowKey: `ending:${type}:${lang}`,
        deleteEndpoint: "__ending-voice/delete", deleteBody: { endingType: type },
      }),
    }));
  ...
  for (const milestone of BUNDLED_MILESTONES) {
    ...
    headPick.textContent = getCurrentLanguage() === "spanish"
      ? `En juego: #${picked}`
      : `In game: #${picked}`;
...
function buildLanguageToggle() {
  const wrap = document.createElement("div"); wrap.className = "voice-tab-language";
  const label = document.createElement("span"); label.className = "voice-tab-language__label";
  label.textContent = getCurrentLanguage() === "spanish" ? "Idioma" : "Language";
  wrap.appendChild(label);
  const group = document.createElement("div"); group.className = "voice-tab-language__group";

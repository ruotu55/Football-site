...
async function readyPhotoVariantExists(playerName, clubName, variantIndex) {
  ...
  for (const rel of careerReadyPhotoRelCandidatesForStem(playerName, clubName ?? "", stem)) {
    ...
    /* Some static hosts mis-handle HEAD; fall back to the same load probe used for display. */
    try {
      const img = await preloadImage(url);
...
export function renderCareer() {
  ...
  wrap.innerHTML = "";
  /* Orphan URL modal if a prior render destroyed its host while the dialog was portaled to `body`. */
  document.body
  ...
  /* Hide during per-level Video mode and during Play Video (`isVideoPlaying`): wrap may drop
   * `.video-mode-enabled` while `body.play-video-active` is on, so do not rely on videoMode alone. */
  ...
  revealFallback.textContent = hasRealPlayer
    ...
    if (isSingleTeamRoad) {
      ...
      roadEndX = centerX + halfRoadLength;
      roadY = centerY;
      d = `M ${centerX - halfRoadLength},${centerY} L ${centerX + halfRoadLength},${centerY} `;
    } else {
      for (let i = 0; i < n; i++) {
      ...
      roadStartX = points[0].x;
      roadEndX = points[n - 1].x;
      roadY = points[0].y;

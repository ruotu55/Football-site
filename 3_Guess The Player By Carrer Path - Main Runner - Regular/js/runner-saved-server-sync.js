        ...
        flushLocalAndServer(scripts, folders, folderStates) {
            const safeSet = (key, value) => {
                try {
                    ...
                    /* QuotaExceededError or similar — server sync below is the source of truth,
                       so don't fail the caller. Log once for diagnostics. */
                    if (typeof console !== "undefined" && console.warn) {
                        console.warn("[runner-saved-server-sync] localStorage write failed for " + key + ":", e?.name || e);
                    }
                }
            };
            safeSet(keys.KEY_SCRIPTS, JSON.stringify(scripts));
            safeSet(keys.KEY_FOLDERS, JSON.stringify(folders));
            safeSet(keys.KEY_FOLDER_STATES, JSON.stringify(folderStates));
            if (!active) return;

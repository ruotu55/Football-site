...
# Players shared across runners & languages; Endings shared across runners, per-language;
# Quiz Intro per runner + per-language.
RUNNER_VARIANT = "Career Path Regular"
...
QUIZ_TITLE_PROMPT_BY_QUIZ_TYPE = {
    ...
    "spanish": {
        "player-by-career": "Hola a todos, empecemos. ...\n\n¡¡ADIVINA AL JUGADOR POR TRAYECTORIA!!",
        "player-by-career-stats": "Hola a todos, empecemos. ...\n\n¡¡ADIVINA AL JUGADOR POR TRAYECTORIA!!",
    },
}
...
def _fetch_bytes(
    ...
    *,
    ...
    headers: dict[str, str] = {"User-Agent": HTTP_USER_AGENT, "Accept": "*/*"}
    if extra_headers:
...
def _player_voice_paths_for_name(player_name: str, language: str | None = None) -> list[Path]:
    """Players share one folder across all runners & languages."""
    del language
...
def _player_voice_prompt(player_name: str, phrase: str, language: str) -> str:
    """Render the reveal sentence and dial up the excitement: trailing exclamation so
    the TTS engine punches the delivery, plus Spanish inverted '¡' opener when needed."""
...
class RunnerRequestHandler(SimpleHTTPRequestHandler):
    ...
    def _try_serve_player_voice_status(self) -> bool:
        ...
        try:
            ...
            return True
        existing_path = None
        for file_path in _player_voice_paths_for_phrase(player_name, target_dir):
        ...
        if existing_path is None and language == "english" and phrase == "plain":
            for file_path in _player_voice_legacy_root_paths(player_name):
                if file_path.is_file():
                    existing_path = file_path
                    break
        self._send_json(
    ...
    def _try_generate_player_voice(self) -> bool:
        ...
        for old_path in _player_voice_paths_for_phrase(player_name, target_dir):

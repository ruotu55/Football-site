// js/saved-scripts.js — Main Runner - Player Name - Regular (storage isolated from other runners)
import {
    appState,
...
const VOICE_FREEZE_MIGRATION_FLAG = "footballQuizVoiceFreezeMigrated_player_name_regular_v2";
...
const KEY_SCRIPTS = "footballQuizScripts_player_name_regular_v1";
const KEY_FOLDERS = "footballQuizFolders_player_name_regular_v1";
const KEY_FOLDER_STATES = "footballQuizFolderStates_player_name_regular_v1";
...
const SAVE_SERVER = createSavedScriptsServerSync("player_name_regular", {
    KEY_SCRIPTS,
    KEY_FOLDERS,
    KEY_FOLDER_STATES,
});
 ...
 *  decides what to do with the returned script. Ambiguous matches are resolved
 *  by taking the single best (longest-key) candidate, since this headless path
 *  has no disambiguation modal. */
export async function buildScriptFromImportText(text, name) {
    ...
    if (importTeams) {
        ...
        disambigItems = clubs;
        disambigDisplayFn = (c) => `${c?.name || "?"} — ${c?.country || "?"} / ${c?.league || "?"}`;

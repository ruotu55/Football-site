...
const TEAM_LOGO_NAME_OVERRIDES_KEY = "team_logo_name_overrides_v1";
let teamLogoNameOverridesCache = null;

function readTeamLogoNameOverrides() {
  if (teamLogoNameOverridesCache) return teamLogoNameOverridesCache;
  let parsed = {};
  try {
    parsed = JSON.parse(localStorage.getItem(TEAM_LOGO_NAME_OVERRIDES_KEY) || "{}");
  } catch {
    parsed = {};
  }
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) parsed = {};
  teamLogoNameOverridesCache = parsed;
  return teamLogoNameOverridesCache;
}

function writeTeamLogoNameOverride(originalName, customName) {
  const orig = String(originalName || "").trim();
  const next = String(customName || "").trim();
  if (!orig || !next) return;
  const overrides = readTeamLogoNameOverrides();
  if (orig === next) {
    delete overrides[orig];
  } else {
    overrides[orig] = next;
  }
  try {
    localStorage.setItem(TEAM_LOGO_NAME_OVERRIDES_KEY, JSON.stringify(overrides));
  } catch {}
}

function resolveTeamLogoNameWithOverride(originalName) {
  const overrides = readTeamLogoNameOverrides();
  let cur = String(originalName || "").trim();
  for (let i = 0; i < 10 && overrides[cur]; i += 1) cur = overrides[cur];
  return cur;
}

function applyTeamLogoNameOverrideToPlayer(careerPlayer) {
  if (!careerPlayer || typeof careerPlayer !== "object") return;
  const cur = String(careerPlayer.club || careerPlayer.name || "").trim();
  if (!cur) return;
  const resolved = resolveTeamLogoNameWithOverride(cur);
  if (resolved && resolved !== cur) {
    careerPlayer.club = resolved;
    careerPlayer.name = resolved;
  }
}

function applyTeamLogoNameOverridesToAllLevels() {
  const levels = Array.isArray(appState?.levelsData) ? appState.levelsData : [];
  for (const lvl of levels) {
    if (lvl && lvl.careerPlayer) applyTeamLogoNameOverrideToPlayer(lvl.careerPlayer);
  }
}
...
/** object-fit: contain rect for drawing `logoEl` into a wֳ—h canvas (same math as sampleLogoPixelColor). */
...
/** Prefer RAM-cached decoded bitmap ג€” DOM <img> can fail drawImage while [hidden] or mid-decode. */
...
/** Comprehensive lookup for a saved punch snap ג€” used by both attach (so the punch is visible
...
function restoreTeamLogoPunchCanvasFromState(state, punchCanvas, logoImg) {
  ...
  // size and is visible, keep the existing pixels ג€” re-setting width/height clears the
...
function attachFakeInfoTeamLogoMaskEditor({
  ...
  // and the timer-end display:none ג†’ block flips re-fires the fade-in keyframe animation,
  ...
  brushSmallerBtn.textContent = "גˆ’";
...
/* Apply Adjust Picture (Up/Down + Width + Height) to the player-card-mode photo.
   ...
   Each Up/Down tick adds ֲ±2%; Width/Height scale stacks multiplicatively from 1.00. */
...
export function renderCareer() {
  ...
  /* Fail-safe: clear any stale unified-visual gate from previous renders before rebuilding. */
  ...
  /* Team-name fake-info mode skips the Get photo / Switch workflow ג€” that card layout
     doesn't host the photo-loader UI. The player-name boxed-card mode does host it
     (buttons sit in the chrome mount above the card). */
  const getPhotoUi =
    hasRealPlayer && !isFakeInfoQuiz()
    ...
    if (hasRealPlayer) {
      if (selectedTeamCardMode) {
        ...
        revealBar.addEventListener("dblclick", async (e) => {
          ...
          const confirmed = await showTeamLogoNameSaveConfirmModal({ oldName: currentDisplay, newName: nextName });
          if (confirmed !== true) return;
          writeTeamLogoNameOverride(currentDisplay, nextName);
          st.careerPlayer.name = nextName;
          st.careerPlayer.club = nextName;
          try {
            await fetch("/__player-voice/rename", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ oldName: currentDisplay, newName: nextName }),
            });
          } catch {}
          renderCareer();
          renderHeader();
        ...
        /* Player-name quiz: structurally identical to the team-name card. Same DOM, same
           classes, same CSS rules ג€” so the box ends up the exact same size visually. The
  ...
  if (previewPostTimer && teamLogoImgEl && restoredPunch) {
    // Punch fades to opacity 0 ג€” the real logo crossfades in (was opacity:0 behind the punch
